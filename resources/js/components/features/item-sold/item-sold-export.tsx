...
import { Category, SoldItem, Supplier } from "@/types"

Font.register({
  family: "Helvetica",
  fonts: [{ src: "Helvetica" }, { src: "Helvetica-Bold", fontWeight: "bold" }],
})
interface SoldItemsDocumentProps {
  items: SoldItem[]
  categories: Category[]
  suppliers: Supplier[]
  logo?: string | null
...
const SoldItemsDocument = ({
  items,
  categories,
  suppliers,
  logo,
}: SoldItemsDocumentProps) => {
  const getCategoryName = (item: SoldItem): string => {
    if (item.category?.name) return item.category.name
    const match = categories?.find((c) => c.id === item.category_id)
    return match?.name || "—"
  }

  const getSupplierName = (item: SoldItem): string => {
    if (item.supplier?.name) return item.supplier.name
    const match = suppliers?.find((s) => s.id === item.supplier_id)
    return match?.name || "—"
  }
  ...
  return (
    <Document>
      ...
      >
        <USIDocsHeader logo={logo} title="SOLD ITEMS REPORT" />
        ...
        >
          <View style={tw("w-[8%] border-r border-gray-300 p-2")}>
            <Text style={tw("text-[8px] text-center font-bold")}>Sold Qty</Text>
          </View>
          ...
          <View style={tw("w-[8%] border-r border-gray-300 p-2")}>
            <Text style={tw("text-[8px] text-center font-bold")}>Size</Text>
          </View>
          <View style={tw("w-[8%] border-r border-gray-300 p-2")}>
            <Text style={tw("text-[8px] text-center font-bold")}>Color</Text>
          </View>
          <View style={tw("w-[10%] border-r border-gray-300 p-2")}>
...
interface SoldItemsExportProps {
  items: SoldItem[]
  categories: Category[]
  suppliers: Supplier[]
  logo?: string | null
...
export const SoldItemsExport = ({
  items,
  categories,
  suppliers,
  logo,
}: SoldItemsExportProps) => {
  ...
  // 2. Memoize Document to prevent re-renders
  const documentComponent = useMemo(
    ...
    [items, categories, suppliers, optimizedLogo],
  )
  ...
  const handleDownload = () => {
    ...
    link.href = url
    link.download = `sold-items-report-${new Date().toISOString().split("T")[0]}.pdf`
    ...
    return (
      <div className="flex h-[70vh] w-full flex-col items-center justify-center gap-3 rounded-md border border-dashed bg-slate-50">
        ...
        <div className="flex flex-col items-center text-xs text-muted-foreground">
          ...
          <span>Processing {items.length} records</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
      <iframe
        ...
        title="Sold Items Export Preview"
      />
