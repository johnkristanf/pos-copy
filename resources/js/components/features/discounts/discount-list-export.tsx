...
const formatDiscountPdfValue = (amount: number, type: string) => {
...
interface DiscountListDocumentProps {
  discounts: Discount[]
  logo?: string | null
}

const DiscountListDocument = ({
  discounts,
  logo,
}: DiscountListDocumentProps) => {
  return (
    <Document>
      <Page
        size="LETTER"
      ...
      >
        <USIDocsHeader logo={logo} title="DISCOUNT MASTER LIST" />
        ...
        >
          <View style={tw("w-[15%] border-r border-gray-300 p-2")}>
            <Text style={tw("text-[8px] text-center font-bold")}>Name</Text>
          </View>
          ...
          <View style={tw("w-[8%] border-r border-gray-300 p-2")}>
            <Text style={tw("text-[8px] text-center font-bold")}>Min Qty</Text>
          </View>
          <View style={tw("w-[10%] border-r border-gray-300 p-2")}>
            <Text style={tw("text-[8px] text-center font-bold")}>
              Min Spend
          ...
          <View style={tw("w-[10%] border-r border-gray-300 p-2")}>
            <Text style={tw("text-[8px] text-center font-bold")}>Capped</Text>
          </View>
          <View style={tw("w-[17%] p-2")}>
            <Text style={tw("text-[8px] text-center font-bold")}>Validity</Text>
          </View>
        </View>
        ...
        {discounts.map((discount, _index) => (
          ...
          >
            <View style={tw("w-[15%] border-r border-gray-300 p-2")}>
              <Text style={tw("text-[8px]")}>{discount.name}</Text>
            </View>
            <View style={tw("w-[20%] border-r border-gray-300 p-2")}>
            ...
            <View style={tw("w-[8%] border-r border-gray-300 p-2")}>
              <Text style={tw("text-[8px] text-center")}>
                {discount.min_purchase_qty || "—"}
              </Text>
            </View>
            <View style={tw("w-[10%] border-r border-gray-300 p-2")}>
              <Text style={tw("text-[8px] text-center")}>
                {discount.min_spend ? formatCurrency(discount.min_spend) : "—"}
              </Text>
            </View>
            <View style={tw("w-[10%] border-r border-gray-300 p-2")}>
              <Text style={tw("text-[8px] text-center")}>
                {discount.capped_amount
                  ? formatCurrency(discount.capped_amount)
                  : "—"}
              </Text>
            </View>
            <View style={tw("w-[17%] p-2")}>
              <Text style={tw("text-[8px] text-center")}>
                {formatDatePdf(discount.start_date)} -{" "}
                {formatDatePdf(discount.end_date)}
              </Text>
            </View>
          </View>
        ))}
...
interface DiscountListExportProps {
  discounts: Discount[]
  logo?: string | null
}

export const DiscountListExport = ({
  discounts,
  logo,
}: DiscountListExportProps) => {
  ...
  const documentComponent = useMemo(
    () => <DiscountListDocument discounts={discounts} logo={optimizedLogo} />,
    [discounts, optimizedLogo],
  )
  ...
  const handleDownload = () => {
    ...
    link.href = url
    link.download = `discount-list-${new Date().toISOString().split("T")[0]}.pdf`
    ...
    return (
      <div className="flex h-[70vh] w-full flex-col items-center justify-center gap-3 rounded-md border border-dashed bg-slate-50">
        ...
        <div className="flex flex-col items-center text-xs text-muted-foreground">
          ...
          <span>Processing {discounts.length} discounts</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
      <iframe
        ...
        title="Discount List Export Preview"
      />
