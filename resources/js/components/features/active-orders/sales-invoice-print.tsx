...
export const SalesInvoicePrint = ({
  order,
  user,
  isVatEnabled = false,
  ...
  useEffect(() => {
    ...
    const loadAssets = async () => {
      ...
      const signatureUrl =
        (order as any).user?.user_signature || user?.user_signature
  ...
  }, [order, user, optimizedLogo, optimizedSignature])
  ...
  const documentComponent = useMemo(
    ...
    [order, optimizedLogo, optimizedSignature, isVatEnabled],
  )
  ...
  const handleDownload = () => {
    ...
    link.href = url
    link.download = `sales-invoice-${order.id || "preview"}.pdf`
