...
const StockInDocument = ({ stockIns, logo }: StockInDocumentProps) => {
  return (
    <Document>
      <Page
        size="LETTER"
      ...
      >
        <USIDocsHeader logo={logo} title="STOCK IN REPORT" />
        ...
        >
          <View style={tw("w-[22%] border-r border-gray-300 p-2")}>
            <Text style={tw("text-[8px] font-bold")}>Item Description</Text>
          </View>
          <View style={tw("w-[8%] border-r border-gray-300 p-2")}>
            <Text style={tw("text-[8px] text-center font-bold")}>Quantity</Text>
          </View>
          <View style={tw("w-[13%] border-r border-gray-300 p-2")}>
          ...
          <View style={tw("w-[11%] border-r border-gray-300 p-2")}>
        ...
        {stockIns.map((stockIn) => (
          ...
          >
            <View style={tw("w-[26%] border-r border-gray-300 p-2")}>
            ...
            <View style={tw("w-[8%] border-r border-gray-300 p-2")}>
              <Text style={tw("text-[8px] text-center font-medium")}>
                {stockIn.purchased_quantity}
              </Text>
            </View>
            ...
            <View style={tw("w-[13%] border-r border-gray-300 p-2")}>
              <Text style={tw("text-[8px] text-center")}>
                {stockIn.item.category?.name || "Unknown"}
              </Text>
            </View>
            <View style={tw("w-[14%] border-r border-gray-300 p-2")}>
              <Text style={tw("text-[8px] text-center")}>
                {stockIn.item.supplier?.name || "Unknown"}
              </Text>
            </View>
            <View style={tw("w-[14%] border-r border-gray-300 p-2")}>
              <Text style={tw("text-[8px] text-center")}>
                {stockIn.purchased.received_at || "—"}
              </Text>
            </View>
          </View>
        ))}
...
interface StockInExportProps {
  stockIns: StockIn[]
  logo?: string | null
}

export const StockInExport = ({ stockIns, logo }: StockInExportProps) => {
  ...
  const documentComponent = useMemo(
    () => <StockInDocument stockIns={stockIns} logo={optimizedLogo} />,
    [stockIns, optimizedLogo],
  )
  ...
  const handleDownload = () => {
    ...
    link.href = url
    link.download = `stock-in-report-${new Date().toISOString().split("T")[0]}.pdf`
    ...
    return (
      <div className="flex h-[70vh] w-full flex-col items-center justify-center gap-3 bg-slate-50 rounded-md border border-dashed">
        ...
        <div className="flex flex-col items-center text-xs text-muted-foreground">
          <span className="font-medium">Generating Stock In Report...</span>
          <span>Processing {stockIns.length} items</span>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="flex h-[70vh] w-full items-center justify-center text-destructive bg-red-50 rounded-md border border-red-200">
        <div className="text-center">
          <p className="font-medium mb-2">Failed to generate stock in report</p>
  ...
  return (
    <div className="w-full max-w-6xl mx-auto flex flex-col gap-6">
      <iframe
        ...
        title="Stock In Report Export Preview"
      />
