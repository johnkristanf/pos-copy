...
const PriceListDocument = ({ items, logo }: PriceListDocumentProps) => {
  ...
  return (
    <Document>
      ...
      >
        <USIDocsHeader logo={logo} title="PRICE LIST" />
        ...
        >
          ...
          <View style={tw("w-[19%] border-r border-gray-300 p-2")}>
          ...
          <View style={tw("w-[8%] border-r border-gray-300 p-2")}>
            <Text style={tw("text-[8px] text-right font-bold")}>
              Unit Price
            </Text>
          </View>
          <View style={tw("w-[8%] border-r border-gray-300 p-2")}>
            <Text style={tw("text-[8px] text-right font-bold")}>Wholesale</Text>
          </View>
          <View style={tw("w-[8%] border-r border-gray-300 p-2")}>
            <Text style={tw("text-[8px] text-right font-bold")}>Credit</Text>
          </View>
          ...
          >
            <View style={tw("w-[10%] border-r border-gray-300 p-2")}>
              <Text style={tw("text-[8px]")}>
                {item.sku && (
                  <Text style={tw("text-gray-500")}> {item.sku}</Text>
                )}
              </Text>
            </View>
            <View style={tw("w-[19%] border-r border-gray-300 p-2")}>
            ...
            <View style={tw("w-[8%] border-r border-gray-300 p-2")}>
              <Text style={tw("text-[8px] text-right")}>
                {formatPrice(item.selling_prices?.unit_price)}
              </Text>
            </View>
            <View style={tw("w-[8%] border-r border-gray-300 p-2")}>
              <Text style={tw("text-[8px] text-right")}>
                {formatPrice(item.selling_prices?.wholesale_price)}
              </Text>
            </View>
            <View style={tw("w-[8%] border-r border-gray-300 p-2")}>
              <Text style={tw("text-[8px] text-right")}>
                {formatPrice(item.selling_prices?.credit_price)}
              </Text>
            </View>
            <View style={tw("w-[20%] p-2")}>
              <Text style={tw("text-[8px] text-center")}>
...
export const PriceListExport = ({ items, logo }: PriceListExportProps) => {
  ...
  const documentComponent = useMemo(
    () => <PriceListDocument items={items} logo={optimizedLogo} />,
    [items, optimizedLogo],
  )
    ...
    return (
      <div className="flex h-[70vh] w-full flex-col items-center justify-center gap-3 bg-slate-50 rounded-md border border-dashed">
        ...
        <div className="flex flex-col items-center text-xs text-muted-foreground">
          <span className="font-medium">Generating Price List...</span>
    ...
    return (
      <div className="flex h-[70vh] w-full items-center justify-center text-destructive bg-red-50 rounded-md border border-red-200">
        <div className="text-center">
          <p className="font-medium mb-2">Failed to generate price list</p>
  ...
  return (
    <div className="w-full max-w-6xl mx-auto flex flex-col gap-6">
      <iframe
        ...
        title="Price List Export Preview"
      />
