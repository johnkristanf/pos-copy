...
interface CustomerListDocumentProps {
  customers: Customer[]
  logo?: string | null
}

const CustomerDocument = ({ customers, logo }: CustomerListDocumentProps) => {
  return (
    <Document>
      <Page
        size="LETTER"
      ...
      >
        <USIDocsHeader logo={logo} title="CUSTOMER LIST" />
        ...
        >
          ...
          <View style={tw("w-[20%] border-r border-gray-300 p-2")}>
            <Text style={tw("text-[8px] text-center font-bold")}>Contact</Text>
          </View>
          <View style={tw("w-[20%] border-r border-gray-300 p-2")}>
        ...
        {customers.map((customer) => {
          ...
          return (
            ...
            >
              ...
              <View style={tw("w-[25%] border-r border-gray-300 p-2")}>
                <Text style={tw("text-[8px]")}>{customer.name}</Text>
              </View>
              <View style={tw("w-[20%] border-r border-gray-300 p-2")}>
              ...
              <View style={tw("w-[20%] border-r border-gray-300 p-2")}>
                <Text style={tw("text-[8px]")}>{address}</Text>
              </View>
              <View style={tw("w-[10%] border-r border-gray-300 p-2")}>
                <Text style={tw("text-[8px] text-center")}>
                  {customer.affiliated ? "Affiliated" : "Non-Affiliated"}
                </Text>
              </View>
              <View style={tw("w-[15%] p-2")}>
                <Text style={tw("text-[8px] text-right")}>
                  {formatPdfNumber(customer.total_order_value || 0)}
                </Text>
              </View>
            </View>
          )
...
interface CustomerListExportProps {
  customers: Customer[]
  logo?: string | null
}
...
export const CustomerExport = ({
  ...
  const documentComponent = useMemo(
    () => <CustomerDocument customers={customers} logo={optimizedLogo} />,
    [customers, optimizedLogo],
  )
  ...
  const handleDownload = () => {
    ...
    link.href = url
    link.download = `customer-list-${new Date().toISOString().split("T")[0]}.pdf`
    ...
    return (
      <div className="flex h-[70vh] w-full flex-col items-center justify-center gap-3 bg-slate-50 rounded-md border border-dashed">
        ...
        <div className="flex flex-col items-center text-xs text-muted-foreground">
          ...
          <span>Processing {customers.length} customers</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="w-full max-w-6xl mx-auto flex flex-col gap-6">
      <iframe
        ...
        title="Customer List Export Preview"
      />
